...
from config.constants import TARGET_CHILD_USERNAME, ChronicleDeviceType, Column, TimestampFormat
...
class ColumnPreprocessor(BasePreprocessor):
    ...
    def create_additional_columns(self, df: pd.DataFrame, device_model: ChronicleDeviceType) -> pd.DataFrame:
        """
        ...
        df_copy[Column.DATETIME_OF_PREPROCESSING] = datetime_class.now().strftime(TimestampFormat.DATETIME)
        ...
        if Column.EVENT_TIMESTAMP in df_copy.columns:
            ...
            df_copy[Column.WEEKDAY_MF] = (df_copy[Column.EVENT_TIMESTAMP].dt.weekday < 5).astype(int)
            df_copy[Column.WEEKDAY_MTH] = (df_copy[Column.EVENT_TIMESTAMP].dt.weekday < 4).astype(int)
            df_copy[Column.WEEKDAY_SUTH] = (
                (df_copy[Column.EVENT_TIMESTAMP].dt.weekday < 4) | (df_copy[Column.EVENT_TIMESTAMP].dt.weekday == 6)
            ).astype(int)
        ...
        LOGGER.debug(f"Successfully created {len(df_copy.columns) - len(df.columns)} additional columns")
        return df_copy
